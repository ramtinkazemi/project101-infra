...
# bash $bin_path/apply.sh "$config_path" -destroy $@
...
### check approved plan 
[ -f plans/wrap1 ] || fatal "Plan wrap not found. 'make plan' first."
...
# terragrunt run-all plan -destroy ${TG_PLAN_ARGS} $@
# status=$?
# popd > /dev/null
# [[ $status = 0 ]] || fatal "Failed to generate cross-plans." $status

# pushd $bin_path > /dev/null
# go run wrap.go -colorfull=true | tee $root_path/plans/wrap2
# status=$?
# popd > /dev/null
# [[ $status = 0 ]] || fatal "Failed to generate cross-plan wrap." $status
...
# [[ "$sum1" == "$sum2" ]] || fatal "Plans are stale. Run 'make plan TG_CONFIG=<config/xxx> TG_DESTROY=true' first."
# info "Plans are up to date, proceeding..."

pushd $config_path > /dev/null
terragrunt run-all destroy ${TG_DESTROY_ARGS} $@
status=$?
popd > /dev/null
[[ $status = 0 ]] || fatal "Failed to destroy resources." $status
